...
export function setupAnimation4({ assets = [], jobs = [], theme = null, onChanged, onNotice, initialAssetId = "" } = {}) {
  ...
  root.querySelector("[data-a4-upload]")?.addEventListener("click", () => {
    root.querySelector(".a4-head [data-studio-view='assets']")?.click();
    window.setTimeout(() => document.querySelector("[data-library-import]")?.click(), 0);
  });
  ...
  const live = root.querySelector(".a4-live"); const show = (state) => { live.hidden = false; live.querySelector("[data-a4-status]").textContent = String(state.status || "queued").toUpperCase(); live.querySelector("[data-a4-title]").textContent = state.status === "pending" ? "Generating your complete sequence…" : "Animation is queued"; live.querySelector("[data-a4-detail]").textContent = state.error || (state.model?.includes("+local-rmbg") ? "Cleaning the known magenta matte locally and preserving transparent pixels…" : "The model is sampling all frames as one coherent animation."); live.querySelector("[data-a4-progress]").value = Number(state.progress || 0); };
