...
export function setupAnimation4({ assets = [], jobs = [], theme = null, onChanged, onNotice, initialAssetId = "" } = {}) {
  ...
  root.querySelectorAll("[data-a4-again]").forEach((button) => button.addEventListener("click", () => { if (!button.dataset.a4Again) return; window.location.assign(`/app?view=animation&animate=${encodeURIComponent(button.dataset.a4Again)}`); }));
