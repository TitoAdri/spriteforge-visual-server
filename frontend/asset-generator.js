import { processPixelGrid, quantizePalette } from "/frontend/pixel-grid-core.js";
import { cropForeground, forceGrid, removeChromaBleed, removeChromaKey } from "/frontend/character-creator.js?v=17";
...
const MODES = [
  { id: "item", type: "item", title: "Item", subtitle: "Inventory & collectibles", image: "/assets/examples/asset-generator/item.png", brief: "A small brass compass with a glowing blue needle and worn leather loop", view: "left-3-4", size: 48, tags: ["rpg", "adventure"] },
  { id: "weapon", type: "weapon", title: "Weapon / tool", subtitle: "Equipable silhouette", image: "/assets/examples/asset-generator/tool-weapon.png", brief: "A weathered copper lantern staff with a crystal core", view: "left-3-4", size: 64, tags: ["fantasy", "rpg"] },
  { id: "prop", type: "prop", title: "Gameplay prop", subtitle: "Interactable objects", image: "/assets/examples/asset-generator/gameplay-prop.png", brief: "An old oak treasure chest with iron bands, keyhole and a folded map beside it", view: "left-3-4", size: 64, tags: ["rpg", "dungeon crawler"] },
  { id: "environment", type: "environment", title: "Scenery", subtitle: "World elements", image: "/assets/examples/asset-generator/scenery.png", brief: "A mossy stone shrine with a small hanging lantern and roots around the base", view: "left-3-4", size: 72, tags: ["fantasy", "mystical"] },
  { id: "building", type: "building", title: "Building", subtitle: "Standalone structures", image: "/assets/examples/asset-generator/building.png", brief: "A compact witch's apothecary with crooked roof, glowing windows and a wooden sign", view: "isometric", size: 96, tags: ["fantasy", "cozy"] },
  { id: "ui-icon", type: "ui-icon", title: "UI icon", subtitle: "Game controls & status", image: "/assets/examples/asset-generator/ui-icon.png", brief: "A bold emerald potion icon for a fantasy game health menu", view: "front", size: 48, tags: ["rpg", "handheld retro"] },
];
...
let clean = variant === "v2-transparent" ? snapped : removeChromaBleed(removeChromaKey(snapped));
if (state.colors != null) clean = quantizePalette(clean, state.colors, 16);
finalImage = variant === "v2-transparent" ? clean : removeChromaBleed(removeChromaKey(clean));
...
const body = { provider: root.querySelector("#asset-provider").value, tier: "draft", themeId: theme?.id || null, ...(variant === "v2-transparent" ? { pixelArtVariant: "asset-transparent-v2" } : {}), recipe: { assetType: state.mode.type, subject: root.querySelector("#asset-subject").value.trim(), view: root.querySelector("#asset-view").value, lockView: true, pose: "static asset presentation", target: { width: size, height: size }, palette: { maxColors: state.mode.type === "ui-icon" ? 16 : 24, mood: "production-ready game pixel art" }, pixelScale: root.querySelector("#asset-pixel-scale").value, styleTags: [...state.tags], composition: { paddingPercent: state.mode.type === "building" ? 10 : 18, fullBody: false, groundShadow: false }, lockedTraits: [] } };
setScreen("loading");
...
render();
  ...
  root.querySelector("#asset-download").insertAdjacentHTML("beforebegin", '<button id="asset-view-app" type="button">View in app</button><button id="asset-animate" type="button">Animate</button>');
...
URL.revokeObjectURL(url);
  ...
  root.querySelector("#asset-edit")?.addEventListener("click", () => { if (savedAssetId) window.location.assign(`/app?edit=${encodeURIComponent(savedAssetId)}`); });
  root.querySelector("#asset-view-app").addEventListener("click", () => { if (savedAssetId) window.location.assign(`/app?asset=${encodeURIComponent(savedAssetId)}`); });
  root.querySelector("#asset-animate").addEventListener("click", () => { if (savedAssetId) window.location.assign(`/app?animate=${encodeURIComponent(savedAssetId)}`); });
}
