...
import { appStudioMarkup, setupAppStudio } from "/app-studio.js?v=106";
...
function render() {
  ...
  if (path === "/verify-email") {
    ...
    fetch("/api/auth/verify-email", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ token }) }).then(async (response) => {
      ...
      status.textContent = "Verified — your 40 free Forge credits are ready."; status.dataset.state = "success"; window.spriteforgeTrackXOnce?.("SignUp", { status: "completed" }); history.replaceState({}, "", "/verify-email");
