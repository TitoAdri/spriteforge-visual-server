...
export const assetGeneratorMarkup = ({ embedded = false, theme = null, isEditorAdmin = false } = {}) => {
...
export function setupAssetGenerator({ theme = null, isEditorAdmin = false, onSaved } = {}) {
  const root = document.querySelector("#asset-generator");
if (!root) return;
  if (!isEditorAdmin) root.querySelector("#asset-edit")?.remove();
