...
No subir nunca `generator/.env`, `generator/data/`, bases SQLite, backups ni resultados generados. El repositorio los excluye mediante `.gitignore`. Para desplegar producción hay que conservar el `generator/.env` y el volumen `/srv/spriteforge/generator/data` del servidor; un `git pull` no debe sobrescribirlos.
...
```bash
ssh deploy@your-server.example
cd /srv/spriteforge
...
- En producción guarda copias en `/srv/spriteforge-backups` y conserva 14 días.
